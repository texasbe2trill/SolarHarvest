...
// Colour for a reflective MIP display.
//
...
//
//  * Every colour sits on the 0x00/0x55/0xAA/0xFF channel grid, which is Garmin's
//    64-colour device palette. Off-grid values get dithered into a stipple that
//    looks like dirt on a 280px screen.
//  * Fully saturated hues only. Pastels and tints collapse towards grey.
...
class Palette {
    ...
    // object with its stroke rather than as grey furniture.
    ...
    // dead, so a reading of 80% arrived as an alarm colour. Both ramps now run
    // cool-and-weak to hot-and-strong, which is the direction the metaphor
    // already sets. Everything stays on the 64-colour grid and dark enough to
    // read on white.
    ...
    //
    // This used to colour the caret off the intensity ramp, which made magenta
    ...
    // conflicting meanings for one colour. Direction gets the same two tokens
